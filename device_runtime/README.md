...
- For the validated Raspberry setup, keep `DEVICE_BUTTON_ADAPTER=whisplay` and leave `DEVICE_AUDIO_IN_ADAPTER` / `DEVICE_AUDIO_OUT_ADAPTER` at `null` unless you have confirmed a non-conflicting external audio path.

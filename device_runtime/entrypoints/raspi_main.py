...
from device_runtime.application.ports import BackendGateway, StateObserver
...
class RuntimeObserver(StateObserver):
    ...
    def publish(self, snapshot: DeviceSnapshot) -> None:
        ...
        model = self._display_model_service.build(snapshot)
        self._runtime.display.render(model)
...
class RuntimeTransportGateway(BackendGateway):
    ...
    async def stop_listen(self, turn_id: str) -> None:
        await self._transport.send(build_message("recording.stop", turn_id=turn_id))

    async def cancel_listen(self, turn_id: str | None) -> None:
        payload: dict[str, Any] = {}
        if turn_id:
            payload["turn_id"] = turn_id
        await self._transport.send(build_message("recording.cancel", **payload))

    async def send_audio_chunk(self, turn_id: str, chunk: dict[str, Any]) -> None:
        await self._transport.send(build_message("audio.chunk", turn_id=turn_id, **chunk))

    async def request_agents_version(self) -> None:
        await self._transport.send(build_message("agents.version.request"))

    async def request_agents_list(self) -> None:
        await self._transport.send(build_message("agents.list.request"))

    async def confirm_agent(self, agent_id: str) -> None:
        await self._transport.send(build_message("agent.select", agent_id=agent_id))
...
def build_runtime(env: dict[str, str] | None = None) -> RuntimeBootstrap:
    ...
    button, button_capability = _resolve_button(config)
...
def _resolve_button(config: RuntimeConfig) -> tuple[Any, CapabilityState]:
